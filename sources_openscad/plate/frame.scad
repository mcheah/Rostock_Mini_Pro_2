...
module lower_frame_3D(height) difference() {
	minkowski() {
		intersection() {
			cylinder(r=frame_rad,h=height/2,$fn=6);
            echo(1*2/sqrt(3));
			color("red") rotate(90) cylinder(r=frame_rad*1.3,h=height/2,$fn=3);
//            color("green") circle(r=frame_radius*1.3,$fn=120);
		}
		cylinder(r=corn,h=height/2);
	}
	// mounting holes PCB
	translate([0,-10,0])
//	for(x = [-1, 1]) for(y = [-1, 1]) 
...
//		circle(r=1.2, $fn = 12);
	...
	#translate([0,printbed[1]/4,0]) minkowski() {
		cube([6,printbed[1]/2,height*2],center=true);
		cylinder(r=5,h=height*2,center=true);
	}
...
//		circle(r=1.2, $fn = 12);
    ...
    color("green")
	for(i = [0:2]) rotate(i*120)
	translate([0,tower_rad+2.4,0])rotate(180)  mounting_screws_3D(4,height+2);
...
module upper_frame_3D(height) difference() {
	minkowski() {
		intersection() {
			cylinder(r=frame_rad,h=height/2,$fn=6);
			rotate(90) cylinder(r=frame_rad*1.3,h=height/2,$fn=3);
		}
		cylinder(r=corn,h=height/2,);
	}
	// mounting screws for idler mounts
...
//	// inside opening 60% of outside dimensions
	#scale([0.25,0.25,4.0]) translate([0,0,-height/2]) minkowski() {
		intersection() {
			cylinder(r=(tower_radius-rod_diameter/2-corner_radius)/cos(30),h=height/2,$fn=6);
			rotate(90) cylinder(r=(tower_radius-rod_diameter/2-corner_radius)/cos(30)*1.3,h=height/2,$fn=3);
		}
		cylinder(r=corner_radius,h=height/2);
	}
    translate([0,-85,0])
    extruder_mounting_screws_3D(4,height+2);
	// zip ties for endstop wiring
	#rotate(30) translate([((frame_rad*2.0*sin(30))+corn)*0.6+zip_tie[1]*2,0,0]) 
		for(x=[-1,1],y=[-0.5,0.5]) translate([zip_tie[1]*x,tower_rad/4*y,0]) cube(concat(zip_tie,height+10),center=true);
...
//upper_frame_3D(5);
lower_frame_3D(5);
translate([tower_rad*1.35,tower_rad*0.7,0]) rotate(180) upper_frame_3D(5);
