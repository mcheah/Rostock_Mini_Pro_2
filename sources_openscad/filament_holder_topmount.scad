...
module groundplate(height,length=30) 
{
    translate([  0,0,height/2]) cube([length,16,height], center=true);
    translate([+length/2,0,height/2]) cylinder(d=16,h=height,center=true);
    translate([-length/2,0,height/2]) cylinder(d=16,h=height,center=true); 
}
...
//                groundplate(6);
        ...
        #translate([-5,  2,   15]) rotate([00,0,0]) cylinder(d=5.1,h=31,center=true);
...
//        #translate([-15, 0, 0]) rotate([00,0,0]) cylinder(d=4.3,h=40,center=true);
        ...
        #translate([-5, +2, 3])    rotate([00,0,0]) cylinder(d=9.9,h=6.1,center=true,$fn=6);
    }
}
//
module clampplate(length=30)
{
    w=33;
    th=8.75+0.5;
    th2=8;
    difference() {
        hull(){
            color("blue") translate([0,-(w)/2,6/2]) cube([length+16,w,6],center=true);
            groundplate(6,length);
        }
        hull(){
        translate([43.5/2,8-20])
        cylinder(d=3.2,h=25);
        translate([43.5/2,8-30])
        cylinder(d=3.2,h=25);
        }
